...
resource "oci_core_instance" "wireguard_eu_instance" {
  compartment_id      = var.compartment_ocid
  display_name        = "wireguard_eu_instance"
  availability_domain = data.oci_identity_availability_domain.wireguard_ad.name
  availability_config {
    recovery_action             = "RESTORE_INSTANCE"
    is_live_migration_preferred = false
  ...
  create_vnic_details {
    ...
    hostname_label            = "wireguard-eu"
    skip_source_dest_check    = "true"
  }
  metadata = {
    user_data           = base64encode(file("./wireguard_eu.sh"))
    ssh_authorized_keys = file(".terraform/authorized_keys")
  }
  freeform_tags = {
    "SERVER_PRIVATE_KEY" = file(".terraform/server_private_key")
